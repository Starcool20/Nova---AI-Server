...
app.post('/prompt-nova', upload.single('audio'), async (req, res) => {
  try {
    ...
    res.send(finalBuffer);
      ...
      const json = JSON.stringify({ response: gptResponse.response, transcript: transcription, packageName: command.packageName, command: command.command, contactName: command.contactName, hour: command.hour, minutes: command.minutes, songName: command.songName, message: command.message, gmail: command.gmail, checkCommand: command.checkCommand }) + "\n";
